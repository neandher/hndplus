...
if (count($captcha) > 0) {
    ...
    if ($find_login[0]->attr['href'] == 'vo-inicio.asp') {
        ...
        if (count($pedidos_efetuados) > 0) {
            ...
            foreach ($pedidos_efetuados as $cdh => $produtos) {

                if (!in_array($cdh, $cdh_check)) {

                    $cdh_info = getCdh($cdh, $db);

                    $cdh_check[] = $cdh;
                    $str .= '<div class="panel panel-primary"><div class="panel-heading">';
                    $str .= '<strong>' . $cdh_info['description'] . ' - ' . $cdh_info['district'] . ' - ' . $cdh_info['state'];
                    $str .= ' - Total de ' . count($pedidos_efetuados[$cdh]) . ' iten(s)</strong></div>';
                }
                ...
                foreach ($produtos as $prod) {
                    ...
                    $sql_prod = getProd($prod['pro_code'], $db);
                    ...
                    $str .= '<tr>
                                ...
                                <td><h4><span class="label label-' . $class_status . '">' . $sql_prod['name'] . '</span></h4></td>
                                <td><h4><span class="label label-' . $class_status . '">' . $sql_prod['code'] . '</span></h4></td>
                                <td><h4><span class="label label-' . $class_status . '">' . $prod['pro_qtd'] . '</span></h4></td>
                                <td><h4><span class="label label-' . $class_status . '">' . $status . '</span></h4></td>
                             </tr>';
                }

                $str .= '</tbody></table>';

                if ($retorno_pedidos[$cdh] <> false) {
                    ...
                    $str .= '<table class="table table-condensed">
                            ...
                            <tbody>
                            <tr>
                                <td><h4><span class="label label-info">' . $retorno_pedidos[$cdh]['numero_pedido'] . '</span></h4></td>
                                <td><h4><span class="label label-info">' . number_format(
                            $retorno_pedidos[$cdh]['valor_total_pedido'],
                            2,
                            ',',
                            '.'
                        ) . '</span></h4></td>
            ...
            echo '<div class="alert alert-danger" role="alert">Nenhum pedido efetaudo!</div>';
        }
    } else {
