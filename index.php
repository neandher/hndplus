...
<?php include('navbar.phtml') ?>
    ...
    <form id="formSearch" class="form-horizontal" onsubmit="return false">
        ...
        <div id="pesquisa_automatica_ativa" class="hide">
            <div class="page-header">
                <h2><i class="glyphicon glyphicon-dashboard"></i> Pesquisa Autom&aacute;tica Ativa</h2>
            </div>
        </div>
        ...
        <div id="loading" class="hide">
            <ul class="list-group" id="result_loading">
    ...
    <script>
        ...
        function executaProcesso(urlAjax) {
            ...
            pesquisa_automatica_ativa = $('#pesquisa_automatica_ativa');

            switch (pesquisa_opcao.val()) {

                case 'pesquisar_visualizar':
                    $('#btn_nova_pesquisa').removeClass('hide');
                    urlAjax = '<?php echo BASE_URL ?>ajaxVerificaProdutos.php';
                    pesquisa_automatica_ativa.addClass('hide');
                    break;
                ...
                case 'pesquisa_automatica':
                    ...
                    //var timeoutID = window.setInterval(executaProcesso, 60000); // 1 minuto
                    var timeoutID = window.setInterval(executaProcesso, 1800000); // 30 minutos

                    urlAjax = '<?php echo BASE_URL ?>ajaxPesquisaEfetuaPedidoAutomatico.php';
                    pesquisa_automatica_ativa.removeClass('hide');
                    break;
            }
                        ...
                        .done(function (data) {
                                ...
                                $.each(data, function (cdh, produtos) {
                                    ...
                                    if (produtos.length > 0) {
                                        ...
                                        for (j = 0; j < cod_franquias_tag.length; j++) {
                                            if (cod_franquias_tag[j].id == cdh) {
                                                fra_text = cod_franquias_tag[j].label;
                                            }
                                        }
                                        ...
                                        for (i = 0; i < produtos.length; i++) {
                                            ...
                                            for (j = 0; j < cod_produtos_tag.length; j++) {
                                                if (cod_produtos_tag[j].id == produtos[i].pro_code) {
                                                    pro_text = cod_produtos_tag[j].label;
                                                }
                                            }
                                            ...
                                            str_result +=
                                                '<tr> ' +
                                                '<th scope="row"> ' +
                                                '<img src="https://online.hinode.com.br/produtos/' + produtos[i].pro_code + '_p.jpg" onerror="this.src=\'web-files/default.jpg\'"> ' +
                                                '</th> ' +
                                                '<td><h4><span class="label label-success">' + pro_text + '</span></h4></td> ' +
                                                '<td><h4><span class="label label-success">' + produtos[i].pro_code + '</span></h4></td> ' +
                                                '<td><h4><span class="label label-success">' + produtos[i].pro_qtd + '</span></h4></td> ' +
                                                '</tr>';
                                        }
                                ...
                                if (pesquisaFinalizada) {
                                    ...
                                    result.append('Pesquisa finalizada com sucesso!');
                                    $('#bnts_options').show();
                                }
                            }
                        );
